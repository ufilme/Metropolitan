...
#define metroLines 4
#define parkingTime 10
using namespace std;


struct underground{
  string line, terminal, stations[maxStations], last;
  int interval[maxStations-1], number_of_stops, nextTime, diff, retard, parkingFlag, interStation, interTime;
} lines[metroLines];
...
void print(underground lines[]){
  ...
  for (int i = 0; i < metroLines; i++){
    if (lines[i].parkingFlag == 1){
      ...
      cout << right << setw(5) << lines[i].line << setw(20) << lines[i].last << setw(20) << lines[i].stations[lines[i].interStation] << setw(20) << lines[i].terminal << setw(30) << "IN STAZIONE\n" << endl;
    }
  }
  flag = 0;
  for (int i = 0; i < metroLines; i++){
    if (lines[i].parkingFlag != 1){
      ...
      cout << right << setw(5) << lines[i].line << setw(20) << lines[i].last << setw(20) << lines[i].stations[lines[i].interStation] << setw(20) << lines[i].terminal << setw(20) << lines[i].diff/3600 << setw(2) << ":" <<setw(2) << lines[i].diff/60 << setw(2) << ":" << setw(2) << lines[i].diff%60 << endl;
    }
  }
}
...
void last_update_load(underground lines[], int &controlFlag){
  string last_station;
  ...
  for (int i = 0; i < metroLines; i++){
    if (getline(data, last_station)){
        ...
        lines[i].last = lines[i].stations[lines[i].interStation];
        lines[i].interStation++;
...
void parking(underground lines[], int p){
  int currentTime = time();
  lines[p].nextTime = parkingTime + currentTime;
}

void worker(underground lines[], int &controlFlag){
  int currentTime;

  currentTime = time();
  //
  for (int i = 0; i < metroLines; i++){
    lines[i].nextTime = lines[i].interval[lines[i].interTime] + currentTime;
  ...
  while(true){
    ...
    for (int i = 0; i < metroLines; i++){
        ...
        if (lines[i].nextTime - currentTime == 0){
          if (lines[i].last == lines[i].terminal){
